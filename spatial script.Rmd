...
escdata.raw<-fread("area 25 escapement data all.csv")
escdata.filt<-fread("area 25 escapement data filtered.csv")%>%select(-V1)
...
swvi.chum<-fread("swvi chum releases filtered.csv")
...
a25.rps<-fread("area 25 chum rps.csv")
...
streaminlet <- fread("CM A25 filtered stream list_with inlets.csv")
...
AIC(rps_dist, rps_rels, rps_dist_rel,lrps_dist, lrps_rels, lrps_dist_rels)%>%arrange(AIC) #rps_dist model selected by AIC and arranged top to bottom
