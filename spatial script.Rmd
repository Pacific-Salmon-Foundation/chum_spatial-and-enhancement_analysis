...
allreleases <- rel %>% 
  select(SPECIES_NAME,STOCK_NAME,FACILITY_NAME,RELEASE_SITE_NAME,RELEASE_STAGE_NAME,TotalRelease,RELEASE_YEAR,REL_CU_NAME) %>%
  group_by(SPECIES_NAME,REL_CU_NAME,STOCK_NAME,RELEASE_SITE_NAME,FACILITY_NAME,RELEASE_STAGE_NAME,RELEASE_YEAR) %>%
  summarise(releases=sum(as.numeric(TotalRelease)),n=n())

swvi.chum <- allreleases %>% 
  filter(SPECIES_NAME=="Chum"&REL_CU_NAME=="SOUTHWEST VANCOUVER ISLAND") %>%
...
#ggplot(swvi.chum,aes(x=RELEASE_YEAR,y=RELEASE_STAGE_NAME,color=RELEASE_STAGE_NAME,size=releases))+
#  geom_point()+
#  theme_bw()+
#  facet_grid(`site-stock`~.,switch="y",scale="free_y")
#ggsave("~/R/PSF Hatchery Review/analysis/spatial/outputs/junk.pdf",height=20,width=8,dpi=600)
```
...
nameslookup$erank <- c("red", "red", "red", "black", "#3399FF", "red",
                       ...
                       "black", "black", "black")
saveRDS(nameslookup, "nameslookup.rds")
...
zlabnames <- zlabs$SYS_NM
zlabnames <- as.data.frame(zlabnames)
zlabnames$order <- c(1:length(zlabnames[,1]))
colnames(zlabnames) <- c("SYS_NM", "order")
zlabnames <- merge(zlabnames, nameslookup, by = "SYS_NM")
zlabnames <- zlabnames %>%
  arrange(order)
zlabranks <- zlabnames$zrank
zlabnames <- zlabnames$shortnames
dendz <- as.dendrogram(clust.dist.cor.z)
dendz %>%
  set("labels_colors", zlabcolours) %>%
  set("labels_cex", 0.65) %>%
  set("labels", zlabnames) %>%
  set("leaves_pch", 19) %>%
  set("leaves_col", zlabranks) %>%
...
swvi.chum <- allreleases %>% 
  data.frame() %>% 
  filter(SPECIES_NAME=="Chum"&REL_CU_NAME%in%"SOUTHWEST VANCOUVER ISLAND") %>%
  mutate("site-stock"=paste0(RELEASE_SITE_NAME,":",STOCK_NAME))%>%
  filter(!is.na(releases))%>%
  arrange(`site-stock`,RELEASE_STAGE_NAME,RELEASE_YEAR)
