...
#load libraries
knitr::opts_chunk$set(echo = TRUE, fig.align="center",fig.pos = 'p')
...
escdata.filt<-fread("data/area 25 escapement data filtered.csv")
...
```{r, AIC Table for RPS candidate models, echo = FALSE, include=FALSE, message = FALSE, warning = FALSE}

streaminlet <- read_csv("data/CM A25 filtered stream list_with inlets.csv")
rpsdat <- read_csv("data/area 25 chum rps raw and clean.csv")
totrel <- readRDS("RDS/totrel.rds")

modeldat <- merge(streaminlet, rpsdat, by = "SYS_NM")

colnames(totrel) <- c("Year", "totreleases")

modeldat <- merge(totrel, modeldat, by = "Year")
...
modeldat <- merge(corcoefs, modeldat, by = "SYS_NM")
...
EP_ESC_TOTREL <- effect_plot(fdist, pred = totreleases, interval = TRUE, plot.points = TRUE, jitter = 0,x.label = "Total releases", y.label = "Log escapement")
...
## Effects plots for top model: log(RPS)

```{r, Effects across all metrics, echo = FALSE, message = FALSE, warning = FALSE, fig.align = "center", fig.height = 5, fig.width = 6, fig.cap = "Plot of effects included in most parsimonious model."}

plot_summs(fdist)+labs(x="log(escapement)")

```

\newpage

```{r single effects plots for log rps, echo = FALSE, message = FALSE, warning = FALSE, fig.align = "center", fig.height = 6, fig.width = 6, fig.cap = "Effects plots of recruits per spawner by year (top) and total releases (bottom)."}

ggarrange(EPC_RPS_YEAR,EPC_RPS_TOTREL,ncol=1)

```

\newpage

## Effects plots for top model: log(escapement)

```{r, Effects plots escapement all factors, echo = FALSE, message = FALSE, warning = FALSE, fig.align = "center", fig.height = 5, fig.width = 6, fig.cap = "Effects plots of Escapement by correlation coefficient."}

plot_summs(c)+labs(x="log(RPS)")
```

\newpage

```{r, Effects plots escapement, echo = FALSE, message = FALSE, warning = FALSE, fig.align = "center", fig.height = 7, fig.width = 6, fig.cap = "Effects plots of log(escapement) by correlation coefficient (top), distance from enhancement (middle) and total releases (bottom)."}
ggarrange(EP_ESC_COR,EP_ESC_DIST,EP_ESC_TOTREL,ncol=1)
