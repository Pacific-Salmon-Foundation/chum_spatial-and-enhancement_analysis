...
write.csv(rps,"data/area 8 chum with rps raw.csv")

```

```{r}

rps.clean<-rps%>%mutate(rps=replace(rps,rps>0,NA))
...
#log rps plots
logrps<-rps%>%select(Year,prepost,logrps,SYS_NM)%>%arrange(SYS_NM,prepost,Year)
