...
``` {r prep rps data}
escdata<-fread("data/area 8 chum escapement data filtered with metrics.csv")%>%select(-V1)

age<-fread("data/age and ERs NC CC PSE.csv")

ageERs<-age%>%filter(SpeciesId=="CM",CU_Name=="Bella Coola-Dean Rivers, BELLA COOLA RIVER-LATE")%>%
