...
glm2 <- glm(logrps ~ year + SYS_NM + distancefromconumakm, data = modeldat)
summary(glm2)

summ(glm1)
summ(glm2)

ggplot(modeldat, aes(distancefromconumakm, logrps)) +
...
intercept <- as.data.frame(rep(glm2$coefficients[1], 24))
ablines <- as.data.frame(c(intercept, slopes))

# Plot isn't right but I need to look at it fresh later to figure out what's wrong
ggplot(modeldat, aes(distancefromconumakm, rps)) +
  geom_point() +
  geom_abline(intercept = ablines$rep.glm2.coefficients.1...24., slope = ablines$slopes) +
  ylim(-10, 30)
