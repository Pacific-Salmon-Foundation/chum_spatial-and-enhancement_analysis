...
write.csv(a25.escdata.filtered,"area 25 escapement data filtered.csv")
...
ggsave("a25 chum escapement filtered streams.png",dpi=600,height=8,width=7)
...
releases.raw<-fread("AllSEPReleases_2021-01-21 with otoliths revised.csv")
...
write.csv(ggdata.rel2,"swvi chum releases filtered.csv")

```


```{r plot releases for area 8}
ggplot(gg.releases,aes(x=RELEASE_YEAR,y=releases,color=RELEASE_STAGE_NAME,group=grp))+
  geom_line()+geom_point()+
  scale_color_brewer(palette="Set1")+
...
``` {r prep rps data}
a25.escdata<-fread("area 25 escapement data filtered.csv")%>%select(-V1)

age<-fread("SCVI_age_2019-04-13.txt")%>%select(-V1)
...
write.csv(a25.rps,"area 25 chum rps.csv")
