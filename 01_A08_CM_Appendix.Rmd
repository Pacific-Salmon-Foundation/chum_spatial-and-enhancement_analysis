...
# log rps ~ dist_km_bel + dist_km_mcl + Year + subinlet
c <- glm(logrps ~ dist_km_bel + dist_km_mcl + Year + subinlet, 
         data = rps.clean)
summary(c)

# log rps ~ dist_km_bel + dist_km_mcl + Year + SYS_NM
d <- glm(logrps ~ dist_km_bel + dist_km_mcl + Year + SYS_NM, 
         data = rps.clean)
summary(d)
...
eb <- glm(esc.log ~ dist_km_bel + dist_km_mcl + SYS_NM + Year, 
         data = rps.clean)
summary(eb)
```

```{r, Weighted mean models, echo = FALSE, message = FALSE, warning = FALSE}
...
effect_plot(eb, pred = SYS_NM, interval = TRUE, plot.points = TRUE, jitter = .25,
