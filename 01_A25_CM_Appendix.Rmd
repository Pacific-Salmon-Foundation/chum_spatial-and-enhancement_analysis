...
streams <- streaminlet %>% select(SYS_NM, inlet, subinlet)
...
cor.z.long$subinlet <- str_to_sentence(cor.z.long$subinlet)
...
# By subinlet
p2 <- ggplot(cor.z.long, aes(x = factor(subinlet), y = zscore, fill = factor(subinlet))) +
  geom_boxplot() + 
  scale_fill_brewer(palette = "Set1") + 
  labs(x = "Subinlet", y = "Correlation in Z-scores", fill = "Subinlet")  +
  theme_bw() +
...
cor.rpslog.long$subinlet <- str_to_sentence(cor.rpslog.long$subinlet)
...
# By subinlet
p4 <- ggplot(cor.rpslog.long, aes(x = factor(subinlet), y = `log(RPS)`, fill = factor(subinlet))) +
  geom_boxplot() + 
  scale_fill_brewer(palette = "Set1") + 
  labs(x = "Subinlet", y = "Correlation in log(RPS)", fill = "Subinlet") +
  theme_bw() +
  scale_x_discrete(guide = guide_axis(n.dodge = 2))

p5 <- ggarrange(p1, p2, p3, p4)

annotate_figure(p5, top = text_grob("Area 25", 
                face = "bold", size = 14))
...
# Pre- and post-1980 for subinlet

streams <- streaminlet %>% select(SYS_NM, inlet, subinlet)

cor.z.pre.long <- pivot_longer(as.data.frame(cor.z.pre), 
                           cols = c(1:24), 
                           names_to = "SYS_NM", 
                           values_to = "zscore") %>% filter(zscore != 1)
cor.z.pre.long <- merge(cor.z.pre.long, streams, by = "SYS_NM")
cor.z.pre.long$inlet <- str_to_sentence(cor.z.pre.long$inlet)
cor.z.pre.long$subinlet <- str_to_sentence(cor.z.pre.long$subinlet)

cor.z.post.long <- pivot_longer(as.data.frame(cor.z.post), 
                           cols = c(1:24), 
                           names_to = "SYS_NM", 
                           values_to = "zscore") %>% filter(zscore != 1)
cor.z.post.long <- merge(cor.z.post.long, streams, by = "SYS_NM")
cor.z.post.long$inlet <- str_to_sentence(cor.z.post.long$inlet)
cor.z.post.long$subinlet <- str_to_sentence(cor.z.post.long$subinlet)

cor.z.pre.long$prepost <- "Pre-1980"
cor.z.post.long$prepost <- "Post-1980"

prepost.cor.z <- rbind(cor.z.pre.long, cor.z.post.long)

p6 <- ggplot(prepost.cor.z, aes(x = factor(subinlet), y = zscore, fill = factor(prepost), group_by(prepost))) +
  geom_boxplot() + 
  scale_fill_brewer(palette = "Set1") + 
  labs(x = "Subinlet", y = "Correlation in Z-scores", fill = "Period")  +
  theme_bw()




cor.logrps.pre.long <- pivot_longer(as.data.frame(cor.logrps.pre), 
                           cols = c(1:24), 
                           names_to = "SYS_NM", 
                           values_to = "logrps") %>% filter(logrps != 1)
cor.logrps.pre.long <- merge(cor.logrps.pre.long, streams, by = "SYS_NM")
cor.logrps.pre.long$inlet <- str_to_sentence(cor.logrps.pre.long$inlet)
cor.logrps.pre.long$subinlet <- str_to_sentence(cor.logrps.pre.long$subinlet)

cor.logrps.post.long <- pivot_longer(as.data.frame(cor.logrps.post), 
                           cols = c(1:24), 
                           names_to = "SYS_NM", 
                           values_to = "logrps") %>% filter(logrps != 1)
cor.logrps.post.long <- merge(cor.logrps.post.long, streams, by = "SYS_NM")
cor.logrps.post.long$inlet <- str_to_sentence(cor.logrps.post.long$inlet)
cor.logrps.post.long$subinlet <- str_to_sentence(cor.logrps.post.long$subinlet)

cor.logrps.pre.long$prepost <- "Pre-1980"
cor.logrps.post.long$prepost <- "Post-1980"

prepost.cor.logrps <- rbind(cor.logrps.pre.long, cor.logrps.post.long)

p7 <- ggplot(prepost.cor.logrps, aes(x = factor(subinlet), y = logrps, fill = factor(prepost), group_by(prepost))) +
  geom_boxplot() + 
  scale_fill_brewer(palette = "Set1") + 
  labs(x = "Subinlet", y = "Correlation in log(RPS)", fill = "Period")  +
  theme_bw()

p8 <- ggarrange(p6, p7, nrow = 2, common.legend = TRUE, legend = "right")

annotate_figure(p8, top = text_grob("Area 25", 
                face = "bold", size = 14))
...
#png(file = "figures/fig02pairwise_correlations_outliers.png", width = 1024, height = 768, pointsize = 48)
...
model.logrps <- nls(logrps ~ a * distance ^ b, 
               data = dist_logrps,
               start = list(a = 1, b = 1))
...
#png(file = "figures/fig03pairwise_logrps.png", width = 1024, height = 768, pointsize = 48)
b<- ggplot(logrpsplot, aes(distance/1000, logrps)) +
  geom_point() + theme_bw()+
  geom_line( aes(y = logrpspreds), size = 1) +
  geom_label_repel(aes(label = plotlabels),
                   data = subset(logrpsplot, logrps < 0.3),  # These lines label negative outliers
                   size = 2.5) +
...
```{r Distance dendrogram, message = FALSE, warning = FALSE, echo = FALSE, fig.align = "center", fig.height = 8, fig.width = 7, fig.cap = "Dendrogram of pairwise distance between stream mouths. Red labels - Esperanza Inlet; Blue - Nootka Inlet; Green - Tahsis Inlet"}
...
#png(file = "figures/fig05distances_dendrogram.png", width = 800, height = 600, pointsize = 12)
dendd %>%
...
model.zpre <- nls(zscore ~ a * distance ^ b, 
               data = dist_corr.pre,
               start = list(a = 1, b = 1))
...
#png(file = "figures/fig06pairwise_zscore_pre.png", width = 1024, height = 768, pointsize = 48)
#ggplot(dist_corr.pre, aes(distance/1000, zscore)) +
#  geom_point() +
#  geom_line( aes(y = zprepreds), size = 1) +
#  labs(x = "Pairwise distance (km)", y = "Pairwise correlation (z-scores pre-1980)")
#dev.off()
...
model.zpost <- nls(zscore ~ a * distance ^ b, 
               data = dist_corr.post,
               start = list(a = .1, b = .1))
...
#png(file = "figures/fig07pairwise_zscore_post.png", width = 1024, height = 768, pointsize = 48)
#ggplot(zpostplot, aes(distance/1000, zscore)) +
#  geom_point() +
#  geom_line( aes(y = zpostpreds), size = 1) +
#  labs(x = "Pairwise distance (km)", y = "Pairwise correlation (z-scores post-1980)")
#dev.off()
...
model.logrpspre <- nls(logrpsscore ~ a * distance ^ b, 
               data = dist_corr.pre,
               start = list(a = 1, b = 1))
...
#png(file = "figures/fig08_pairwise_logRPS.png", width = 1024, height = 768, pointsize = 48)
#ggplot(dist_corr.pre, aes(distance/1000, logrpsscore)) +
#  geom_point() +
#  geom_line( aes(y = logrpsprepreds), size = 1) +
#  labs(x = "Pairwise distance (km)", y = "Pairwise correlation (Log RPS pre-1980)")
#dev.off()
...
model.logrpspost <- nls(logrpsscore ~ a * distance ^ b, 
               data = dist_corr.post,
               start = list(a = 1, b = 1))
...
#png(file = "figures/fig09_pairwise_logRPS.png", width = 1024, height = 768, pointsize = 48)
#ggplot(logrpspostplot, aes(distance/1000, logrpsscore)) +
#  geom_point() +
#  geom_line( aes(y = logrpspostpreds), size = 1) +
#  labs(x = "Pairwise distance (km)", y = "Pairwise correlation (Log RPS pre-1980)")
#dev.off()
...
#png(file = "figures/fig11pairwise_logRPS.png", width = 1024, height = 768, pointsize = 64)
...
```{r, Effects across all metrics, echo = FALSE, message = FALSE, warning = FALSE, fig.align = "center", fig.height = 5, fig.width = 8, fig.cap = "Plot of effects included in most parsimonious model."}


effectsplot <- readRDS("RDS/plotsumms.rds")
plot(effectsplot)
